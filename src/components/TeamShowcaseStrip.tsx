...
const TeamConnector = ({ side, isDark }: { side: 'left' | 'right'; isDark: boolean }) => {
  ...
  if (side === 'left') {
    return (
      <svg
        aria-hidden="true"
        className="pointer-events-none absolute -right-28 top-[4.4rem] hidden h-[4.75rem] w-28 overflow-visible sm:block"
        viewBox="0 0 112 76"
        fill="none"
      >
  ...
  return (
    <svg
      aria-hidden="true"
      className="pointer-events-none absolute -left-28 top-[4.4rem] hidden h-[4.75rem] w-28 overflow-visible sm:block"
      viewBox="0 0 112 76"
      fill="none"
    >
      ...
      <div className="relative overflow-x-auto pb-4" style={{ scrollbarWidth: 'thin', scrollbarColor: '#52E6C8 transparent' }}>
        ...
        >
          {teamMembers.map((member) => {
            ...
            const tooltipWidth = `clamp(236px, 72vw, ${TEAM_TOOLTIP_WIDTH}px)`;
            const tooltipStyle =
              visual.tooltipSide === 'left'
                ? {
                    right: `calc(100% + ${TEAM_TOOLTIP_OFFSET}px)`,
                    top: '-2px',
                    width: tooltipWidth,
                  }
                : {
                    left: `calc(100% + ${TEAM_TOOLTIP_OFFSET}px)`,
                    top: '-2px',
                    width: tooltipWidth,
                  };
            ...
            return (
              <div
                ...
                style={{ width: `clamp(148px, 38vw, ${TEAM_CARD_WIDTH}px)` }}
              >
                <div
                  role="button"
                  tabIndex={0}
                  ...
                  <AnimatePresence>
                    {isActive && (
                      ...
                      >
                        ...
                        <div className={`rounded-[1.2rem] border px-4 py-4 backdrop-blur-[8px] sm:rounded-[1.45rem] sm:px-6 sm:py-5 ${tone.tooltipClass}`}>
                          <p className={`font-brand text-[0.62rem] font-semibold uppercase tracking-[0.34em] ${tone.eyebrowClass}`}>
                            {copy.eyebrow}
                          </p>
                          <h4 className={`mt-3 font-heading text-2xl font-semibold leading-tight sm:text-[2rem] sm:leading-none ${tone.nameClass}`}>
                            {member.name}
                          </h4>
                          ...
                          <p className={`mt-3 text-sm leading-6 sm:leading-7 ${tone.bioClass}`}>{member.bio[lang]}</p>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
